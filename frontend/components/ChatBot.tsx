...
export default function ChatBot({ currentProductId }: ChatBotProps) {
  const { data: session, status } = useSession();
  const isAdmin = session?.user?.role === "admin";
  const isSignedIn = status === "authenticated";
  const [open, setOpen] = useState<boolean>(Boolean(currentProductId));
  ...
  const seededProductRecommendation = useRef<boolean>(false);
  ...
  async function sendMessage(message: string, silent = false) {
    ...
    setLoading(true);
    try {
      const body = await apiPost<{ answer: string }>("/api/chat", {
