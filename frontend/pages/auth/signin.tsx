...
export default function SignInPage({ error, callbackUrl }: SignInPageProps) {
  ...
  async function handleCredentialsAuth(e: FormEvent) {
    e.preventDefault();
    setFormError("");
    setSubmitting(true);
    ...
    try {
      ...
      if (mode === "signup") {
        ...
        const registerBody = await registerRes.json().catch(() => ({} as Record<string, unknown>));
        if (!registerRes.ok) {
          setFormError((registerBody?.error as string) || "Could not create account.");
          return;
        }
      }
...
export const getServerSideProps: GetServerSideProps<SignInPageProps> = async (context) => {
  const session = await getServerSession(context.req, context.res, authOptions as AuthOptions);
  if (session) {
    return {
      redirect: {
        destination: "/",
