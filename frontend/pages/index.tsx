...
import type { Product as ProductType, ProductDisplay } from "@/types";
...
export default function Home({ products }: HomeProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [products, phrase, selectedCategories, priceMin, priceMax, sortBy, hasSearch]);
    ...
    <Layout>
      ...
      <div className="mt-4 mb-6">
        <Filters
          ...
          priceMin={priceMin}
          priceMax={priceMax}
          onPriceChange={(min: string, max: string) => { setPriceMin(min); setPriceMax(max); }}
          sortBy={sortBy}
                  ...
                  <div>
                    ...
                    <div className="flex -mx-5 overflow-x-scroll snap-x scrollbar-hide">
                      {products.filter(p => p.categories.includes(categoryName))
                        .map(productInfo => (
                          <div key={productInfo.id} className="px-5 shrink-0 snap-start">
                            <Product {...productInfo} />
                          </div>
                        ))}
                    </div>
...
export const getServerSideProps: GetServerSideProps<HomeProps> = async () => {
  try {
    ...
    const rawProducts = await serverApiGet<ProductType[]>('/api/products');

    // Flatten Category objects to string[] for display
    const products: ProductDisplay[] = rawProducts.map(p => ({
      ...p,
